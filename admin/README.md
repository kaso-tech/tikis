...
- Toute action sensible (modification du taux de commission, résolution d'un signalement, consultation d'un profil/litige, suspension d'un admin) est tracée dans `tikis_admin_audit_log`. Comme pour le journal financier, l'immuabilité ne peut pas reposer sur des triggers sur cette base TiDB (voir la note ci-dessus) : elle tient à la même discipline — aucun chemin de code n'émet d'UPDATE ni de DELETE contre cette table — complétée en production par des privilèges DB restreints à INSERT/SELECT.
