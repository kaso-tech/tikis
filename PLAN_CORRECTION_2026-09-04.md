...
## Relecture globale post-implémentation (2026-09-04)

Une relecture de l'ensemble du diff (6 commits, `main..claude/bonjour-0yo56s`) a été effectuée après la Phase 5. Elle a trouvé et corrigé 4 régressions/défauts introduits par les correctifs eux-mêmes, avant tout déploiement :

1. **🔴 Contre-offre de candidature cassée par le nouveau contrôle serveur (régression Phase 1)** — `confirmedCommission` était toujours calculé côté client à partir du prix de base de la livraison, jamais de la contre-offre (`offerPrice`) que le livreur venait de saisir, alors que le serveur calcule désormais la commission sur `offerPrice` quand il est fourni (contrôle ajouté en Phase 1, point 4 du plan). Toute candidature avec contre-offre était donc systématiquement rejetée ("le montant de la commission a changé entre-temps"). Corrigé dans les trois écrans concernés (`app/delivery/[id].tsx`, `components/tikis/screens/home-screen.native.tsx`, `home-screen.web.tsx`) : la commission confirmée est désormais recalculée sur le montant de la contre-offre quand elle existe.
2. **🔴 `requestTikisWalletOperation` bloqué par le trigger d'immuabilité (interaction Phase 2 / Phase 5)** — la clé d'idempotence déterministe ajoutée en Phase 5 utilisait `onDuplicateKeyUpdate`, qui exécute une vraie clause `UPDATE` en cas de relance ; or la Phase 2 a ajouté un trigger `BEFORE UPDATE` qui bloque **toute** modification de `tikis_wallet_ledger`, y compris pour réécrire la même valeur. Une relance réseau de cette demande aurait donc échoué avec une erreur SQL brute. Corrigé en revenant au même schéma que `applyWalletMovement` : vérifier l'existence de la clé avant d'insérer, jamais d'`UPDATE`.
3. **🟡 Cache de résolution d'adresse bloqué après un échec réseau (Phase 3)** — `lastResolvedRef` était mis à jour avant de savoir si le géocodage inverse avait réussi ; un échec ponctuel (réseau, API) laissait ce point marqué "résolu" pour le reste de la session de la modale, empêchant toute nouvelle tentative à moins de 15 m. Corrigé : le point n'est retenu qu'en cas de succès, et réinitialisé en cas d'échec pour permettre une nouvelle tentative immédiate.
4. **🟡 Notification des livreurs compatibles sans limite de lecture SQL (Phase 4)** — la requête chargeait la table entière des livreurs actifs en mémoire (dans la transaction de création de livraison) avant de filtrer et de plafonner en JS ; seul l'accumulateur de résultats était borné, pas la lecture elle-même. Corrigé avec une limite de lecture SQL (2 000 profils les plus récents), documentée comme compromis accepté à très grande échelle.

Suite complète revérifiée après ces 4 correctifs (`npx tsc --noEmit`, `npx vitest run`) : aucune régression supplémentaire, mêmes 3 erreurs TS pré-existantes et 6 échecs de test environnementaux qu'avant.
