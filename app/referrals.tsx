  ...
  return <SafeAreaView style={[styles.safe, { backgroundColor: theme.background }]} edges={["top", "bottom"]}>
    <FlatList
      ...
      ListHeaderComponent={<>
        ...
        <View style={styles.summaryRow}><Metric icon="groups" value={String(referrals.length)} label="Filleuls" theme={theme} /><Metric icon="pending-actions" value={String(qualified)} label="En validation" theme={theme} /><Metric icon="account-balance-wallet" value={formatMoney(walletQuery.data?.total ?? 0)} label="Solde Wallet" theme={theme} /></View>
...
function ReferralRow({ referral, requiredDeliveries, theme, isDark }: { referral: ReferralRecord; requiredDeliveries: number; theme: { primary: string; surface: string; foreground: string; muted: string; success: string; pressed: string; border: string }; isDark: boolean }) {
