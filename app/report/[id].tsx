...
export default function ReportDeliveryScreen() {
  ...
  return <SafeAreaView style={styles.safe} edges={["top", "bottom"]}><KeyboardAvoidingView style={styles.keyboard} behavior={Platform.OS === "ios" ? "padding" : undefined}><ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled"><View style={styles.topBar}><Pressable onPress={() => router.back()} style={({ pressed }) => [styles.back, pressed && styles.pressed]}><MaterialIcons name="arrow-back" size={22} color="#111111" /></Pressable><Text style={styles.topTitle}>Signaler</Text><View style={styles.space} /></View>{sent ? <View style={styles.success}><View style={styles.successIcon}><MaterialIcons name="check-circle" size={32} color="#176C52" /></View><Text style={styles.successTitle}>Signalement envoyé</Text><Text style={styles.successText}>Merci. Votre signalement concernant « {delivery?.title ?? "cette livraison"} » a été transmis à l’administration Tikis et sera conservé dans la chronologie.</Text><TikisButton label="Retour à la livraison" onPress={() => router.back()} style={styles.successButton} /></View> : <><Text style={styles.title}>Aidez-nous à comprendre.</Text><Text style={styles.label}>MOTIF</Text><View style={styles.reasons}>{REASONS.map((item) => <Pressable key={item.value} onPress={() => setReason(item.value)} style={({ pressed }) => [styles.reason, reason === item.value && styles.reasonActive, pressed && styles.pressed]}><Text style={[styles.reasonText, reason === item.value && styles.reasonTextActive]}>{item.label}</Text></Pressable>)}</View><Text style={styles.label}>DÉCRIVEZ LA SITUATION</Text><TextInput value={description} onChangeText={(value) => { setDescription(value); setError(""); }} placeholder="Expliquez ce qui s’est passé…" placeholderTextColor={theme.placeholder} multiline textAlignVertical="top" style={styles.textarea} />{error ? <Text style={styles.error}>{error}</Text> : null}{attachment ? <View style={styles.attach}><Image source={{ uri: attachment.previewUri }} style={styles.attachPreview} /><View style={styles.attachTextWrap}><Text style={styles.attachTitle}>Photo jointe</Text></View><Pressable onPress={() => setAttachment(null)} accessibilityLabel="Retirer la photo" style={({ pressed }) => [styles.attachRemove, pressed && styles.pressed]}><MaterialIcons name="close" size={16} color="#667085" /></Pressable></View> : <Pressable onPress={() => void pickAttachment()} style={({ pressed }) => [styles.attach, pressed && styles.pressed]}><MaterialIcons name="attach-file" size={20} color="#FF9800" /><View style={styles.attachTextWrap}><Text style={styles.attachTitle}>Ajouter une photo (facultatif)</Text></View></Pressable>}<TikisButton label="Envoyer le signalement" icon="send" onPress={() => void send()} loading={reportMutation.isPending} style={styles.submit} /></>}</ScrollView></KeyboardAvoidingView></SafeAreaView>;
}

const baseStyles = StyleSheet.create({
  safe: { flex: 1, backgroundColor: "#F0F3F8" }, keyboard: { flex: 1 }, content: { padding: 20, paddingBottom: 40 }, topBar: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 25 }, back: { width: 42, height: 42, borderRadius: 21, backgroundColor: "#FFFFFF", borderColor: "#F0F3F8", borderWidth: 1, alignItems: "center", justifyContent: "center" }, topTitle: { color: "#1F1206", fontSize: 16, fontWeight: "900" }, space: { width: 42 }, title: { color: "#1F1206", fontSize: 27, fontWeight: "900", letterSpacing: -0.4 }, subtitle: { color: "#667085", fontSize: 14, lineHeight: 21, marginTop: 8 }, label: { color: "#667085", fontSize: 11, fontWeight: "900", letterSpacing: 0.8, marginTop: 28, marginBottom: 10 }, reasons: { flexDirection: "row", flexWrap: "wrap", gap: 8 }, reason: { paddingHorizontal: 13, paddingVertical: 10, backgroundColor: "#FFFFFF", borderRadius: 13, borderColor: "#F0F3F8", borderWidth: 1 }, reasonActive: { backgroundColor: "#FFFFFF", borderColor: "#E3E3E3" }, reasonText: { color: "#667085", fontWeight: "800", fontSize: 12 }, reasonTextActive: { color: "#FF9800" }, textarea: { minHeight: 130, backgroundColor: "#FFFFFF", borderRadius: 16, borderColor: "#E3E3E3", borderWidth: 1, padding: 14, color: "#FF9800", fontSize: 14, lineHeight: 21 }, error: { color: "#A43740", fontSize: 13, fontWeight: "800", marginTop: 8 }, attach: { marginTop: 14, padding: 13, backgroundColor: "#F0F3F8", borderRadius: 15, borderColor: "#F0F3F8", borderWidth: 1, flexDirection: "row", alignItems: "center", gap: 10 }, attachTextWrap: { flex: 1 }, attachTitle: { color: "#FF9800", fontSize: 13, fontWeight: "900" }, attachText: { color: "#FF9800", fontSize: 12, lineHeight: 17, marginTop: 2 }, attachPreview: { width: 40, height: 40, borderRadius: 8 }, attachRemove: { width: 30, height: 30, borderRadius: 15, backgroundColor: "#FFFFFF", alignItems: "center", justifyContent: "center" }, submit: { marginTop: 24 }, success: { alignItems: "center", paddingTop: 58, paddingHorizontal: 15 }, successIcon: { width: 66, height: 66, borderRadius: 24, backgroundColor: "#176C52", alignItems: "center", justifyContent: "center" }, successTitle: { color: "#1F1206", fontSize: 23, fontWeight: "900", marginTop: 18 }, successText: { color: "#667085", fontSize: 14, lineHeight: 21, textAlign: "center", marginTop: 8 }, successButton: { alignSelf: "stretch", marginTop: 26 }, pressed: { opacity: 0.67 },
});

const styles = StyleSheet.create({
  ...baseStyles,
  safe: { ...baseStyles.safe, backgroundColor: "#F0F3F8" },
  ...
  reasonActive: { ...baseStyles.reasonActive, borderWidth: 1, borderColor: "#E3E3E3" },
  reasonText: { ...baseStyles.reasonText, fontWeight: "600" },
  textarea: { ...baseStyles.textarea, borderRadius: 10, borderWidth: 1, borderColor: "#E3E3E3", padding: 13, color: "#FF9800" },
  error: { ...baseStyles.error, fontWeight: "600" },
  attach: { ...baseStyles.attach, borderRadius: 10, borderWidth: 0, backgroundColor: "#F0F3F8", padding: 12 },
