...
export function LiveScreen() {
  ...
  const countByType = useMemo(() => {
    const source = filter === "active" ? deliveries.filter(isActiveStatus) : filter === "upcoming" ? deliveries.filter(isUpcomingStatus) : deliveries.filter((d) => d.status === "completed");
  ...
  const panResponder = useRef(PanResponder.create({
    ...
    onPanResponderMove: (_, gesture) => {
      const current = sheetHeight._value as number;
  ...
  const liveCount = deliveries.filter(isActiveStatus).length;
