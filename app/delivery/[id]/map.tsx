...
export default function DeliveryTrackingScreen() {
  ...
  useEffect(() => {
    // `tension`/`friction` et `stiffness`/`damping`/`mass` sont deux modèles de
    // spring distincts : RN.SpringAnimation refuse d'en mélanger un paramètre.
    Animated.spring(sheetBaseHeight, { toValue: sheetHeightFor(sheetLevel), useNativeDriver: false, tension: 220, friction: 22 }).start();
    panY.setValue(0);
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderTerminate: () => { Animated.spring(panY, { toValue: 0, useNativeDriver: false, tension: 220, friction: 22 }).start(); },
    }),
  ).current;
