...
describe("la tendance de gains et l'historique racontent la même chose", () => {
  it("additionne les gains nets de l'historique, pas le prix brut des courses", () => {
    ...
    const trend = computeDriverEarningsTrend(history, NOW);
    expect(trend.totalLast7Days).toBe(3600);
    expect(trend.averagePerDay).toBe(Math.round(3600 / 7));
  });
  ...
  it("compte les courses publiées sans offre, au lieu de les valoir zéro", () => {
    ...
    // L'historique, lui, retombe sur le prix estimé — la tendance le suit désormais.
    const history = [{ amount: netDriverEarning(1500, 150), createdAt: daysAgo(3) }];
    expect(computeDriverEarningsTrend(history, NOW).totalLast7Days).toBe(1350);
  });
  ...
  it("sépare la semaine écoulée de la précédente pour la tendance", () => {
    ...
    const trend = computeDriverEarningsTrend(history, NOW);
    expect(trend.totalLast7Days).toBe(3000);
    expect(trend.trendPct).toBe(50);
  });
  ...
  it("classe les meilleurs jours sur trente jours, en additionnant les courses d'un même jour", () => {
    ...
    expect(computeDriverEarningsTrend(history, NOW).topDays).toEqual([
  ...
  it("vaut zéro partout sans aucune course, sans inventer de tendance", () => {
    const trend = computeDriverEarningsTrend([], NOW);
    expect(trend).toEqual({ totalLast7Days: 0, averagePerDay: 0, trendPct: null, topDays: [] });
  });

  it("le routeur nourrit la tendance avec l'historique lui-même, pas une seconde requête", () => {
    const routers = read("server/routers.ts");
    expect(routers).toContain("computeDriverEarningsTrend(await db.getDriverCompletedDeliveryEarnings(profile.phone))");
    // Plus aucune agrégation SQL propre à cette carte : c'est elle qui divergeait.
...
describe("l'écran Gains ne se contredit plus", () => {
  ...
  it("l'onglet de la semaine dit ce qu'il filtre", () => {
    ...
    // La carte de tendance, juste au-dessus, emploie ce libellé pour une vraie fenêtre glissante.
  ...
  it("la tendance passe avant le titre de l'historique, pour ne pas s'y confondre", () => {
    expect(screen.indexOf("<DriverEarningsTrend")).toBeLessThan(screen.indexOf("styles.sectionHeader}>"));
  });
});
