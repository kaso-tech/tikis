...
function WelcomeScreen({ onContinue, isDark }: { onContinue: () => void; isDark: boolean }) {
  return <View style={styles.form}>
    ...
    <Text style={[styles.welcomeEyebrow, isDark && { color: AUTH_DARK.amber }]}>BIENVENUE SUR TIKIS</Text>
    ...
    <View style={styles.trustList}>
      <TrustRow icon="verified-user" title="Compte sécurisé" text="Votre numéro est confirmé par un code reçu par SMS." isDark={isDark} />
      <TrustRow icon="handshake" title="Mise en relation transparente" text="Vos coordonnées ne sont partagées qu’une fois la course attribuée." isDark={isDark} />
      <TrustRow icon="payments" title="Vous fixez votre prix" text="Les livreurs l’acceptent ou proposent le leur : vous tranchez." isDark={isDark} />
    </View>

    <TikisButton authStyle label="Commencer" icon="arrow-forward" onPress={onContinue} style={styles.actionButton} />
...
function PhoneScreen({ country, value, error, loading, onCountryPress, onChange, onContinue, isDark }: { country: CountrySpec; value: string; error: string; loading: boolean; onCountryPress: () => void; onChange: (value: string) => void; onContinue: () => void; isDark: boolean }) {
  return <View style={styles.form}>
    <View style={[styles.heroIcon, isDark && { backgroundColor: AUTH_DARK.surface }]}><MaterialIcons name="smartphone" size={30} color={isDark ? AUTH_DARK.amber : "#9A6201"} /></View>
