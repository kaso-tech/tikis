...
const AUTH_DARK = { bg: "#171108", surface: "#231A10", input: "#171108", border: "#4A3823", text: "#FBF7F0", muted: "#C8BCAA", amber: "#D7A447", chip: "#1F1206" };

type Stage = "phone" | "otp" | "role" | "vehicles" | "name";
 ...
 */
function FlowHeader({ creating, step, total, onBack, isDark }: { creating: boolean; step: number; total: number | null; onBack: (() => void) | null; isDark: boolean }) {
  return <View style={styles.top}>
    ...
    <View style={styles.stepInfo}>
      <Text style={[styles.stepLabel, isDark && { color: AUTH_DARK.amber }]}>{creating ? "CRÉATION DU COMPTE" : "VÉRIFICATION DU NUMÉRO"}</Text>
...
/**
 * Le premier écran. Il portait seulement le champ ; l'écran de bienvenue le
 * précédait avec la marque, la promesse et le texte légal. Cet écran-là, un
 * utilisateur qui se reconnecte le retraversait à chaque fois pour n'y rien
 * faire d'autre que continuer — il est fondu ici.
 */
  ...
  return <View style={styles.form}>
    <View style={styles.brandRow}>
      ...
      <View style={styles.brandInfo}>
        ...
        <View style={styles.brandChipRow}>
          ...
          <Text style={[styles.brandChipText, isDark && { color: AUTH_DARK.amber }]}>PLATEFORME DE LIVRAISON</Text>
        </View>
      </View>
    </View>
    ...
    <Pressable accessibilityRole="button" accessibilityLabel={`Pays : ${country.name}. Changer`} onPress={onCountryPress} style={({ pressed }) => [styles.countryField, isDark && { backgroundColor: AUTH_DARK.input, borderColor: AUTH_DARK.border }, pressed && styles.pressed]}>
      ...
      <View style={styles.countryInfo}>
        ...
        <Text style={[styles.countryHint, isDark && { color: AUTH_DARK.muted }]}>{country.dialCode} · {country.digits} chiffres</Text>
      </View>
...
function NameScreen({ role, phone, vehicles, value, error, loading, onChange, onContinue, onEditRole, onEditVehicles, referralCode, referralFieldOpen, onReferralFieldOpen, onReferralCodeChange, isDark }: { role: UserRole | null; phone: string; vehicles: VehicleType[]; value: string; error: string; loading: boolean; onChange: (value: string) => void; onContinue: () => void; onEditRole: () => void; onEditVehicles: () => void; referralCode: string; referralFieldOpen: boolean; onReferralFieldOpen: () => void; onReferralCodeChange: (value: string) => void; isDark: boolean }) {
  return <View style={styles.form}>
    <View style={[styles.heroIcon, isDark && { backgroundColor: AUTH_DARK.surface }]}><MaterialIcons name="badge" size={30} color={isDark ? AUTH_DARK.amber : "#9A6201"} /></View>
    <Text style={[styles.title, isDark && { color: AUTH_DARK.text }]}>Comment devons-nous vous appeler ?</Text>
    <Text style={[styles.subtitle, isDark && { color: AUTH_DARK.muted }]}>{role === "driver" ? "Votre nom sera visible par les expéditeurs après confirmation d’une mission." : "Votre nom sera partagé avec le livreur uniquement après l’attribution d’une course."}</Text>
    ...
    {error ? <Text style={styles.error}>{error}</Text> : <Text style={[styles.helper, isDark && { color: AUTH_DARK.muted }]}>Un nom unique est accepté. Seuls les lettres, espaces, apostrophes et traits d’union sont autorisés.</Text>}
    ...
    {referralFieldOpen ? <>
      ...
      <Text style={[styles.helper, isDark && { color: AUTH_DARK.muted }]}>Un ami vous a parrainé ? Saisissez son code ici.</Text>
    </> : (
      <Pressable accessibilityRole="button" onPress={onReferralFieldOpen} style={({ pressed }) => [styles.referralToggle, pressed && styles.pressed]}>
        <MaterialIcons name="card-giftcard" size={16} color={isDark ? AUTH_DARK.amber : "#9A6201"} />
        <Text style={[styles.referralToggleText, isDark && { color: AUTH_DARK.amber }]}>J’ai un code de parrainage</Text>
      </Pressable>
    )}
    ...
    <View style={[styles.recap, isDark && { backgroundColor: AUTH_DARK.surface, borderColor: AUTH_DARK.border }]}>
      ...
      <View style={[styles.recapRow, styles.recapRowBorder, isDark && { borderBottomColor: AUTH_DARK.border }]}>
        <MaterialIcons name="phone" size={17} color={isDark ? AUTH_DARK.amber : "#9A6201"} />
      ...
      <View style={[styles.recapRow, role === "driver" && styles.recapRowBorder, isDark && { borderBottomColor: AUTH_DARK.border }]}>
        <MaterialIcons name={role === "driver" ? "two-wheeler" : "inventory-2"} size={17} color={isDark ? AUTH_DARK.amber : "#9A6201"} />
        <View style={styles.recapBody}>
        ...
        <Pressable accessibilityRole="button" accessibilityLabel="Modifier le type de compte" onPress={onEditRole} hitSlop={8} style={({ pressed }) => [pressed && styles.pressed]}>
          <Text style={[styles.recapEdit, isDark && { color: AUTH_DARK.amber }]}>Modifier</Text>
        </Pressable>
      </View>
      {role === "driver" ? (
        <View style={styles.recapRow}>
          <MaterialIcons name="local-shipping" size={17} color={isDark ? AUTH_DARK.amber : "#9A6201"} />
          <View style={styles.recapBody}>
    ...
    <TikisButton authStyle label={role === "driver" ? "Créer mon compte livreur" : "Créer mon compte expéditeur"} icon="check-circle" onPress={onContinue} loading={loading} style={styles.actionButton} />
  </View>;
}
...
const baseStyles = StyleSheet.create({
  safeArea: { flex: 1, backgroundColor: "#F0F3F8" }, keyboard: { flex: 1 }, scroll: { flexGrow: 1, padding: 20, paddingBottom: 35 }, brandRow: { flexDirection: "row", alignItems: "center", gap: 12, marginBottom: 26 }, brandLogo: { width: 52, height: 52, borderRadius: 16 }, brandInfo: { flex: 1, minWidth: 0 }, brandName: { color: "#1F1206", fontSize: 17, fontWeight: "900", letterSpacing: -0.2 }, brandChipRow: { flexDirection: "row", alignItems: "center", gap: 6, marginTop: 3 }, phoneTitle: { fontSize: 26, lineHeight: 33 }, recap: { marginTop: 20, borderRadius: 16, backgroundColor: "#FFFFFF", borderWidth: 1, borderColor: "#F0F3F8", paddingHorizontal: 13, paddingBottom: 4 }, recapTitle: { color: "#667085", fontSize: 10, fontWeight: "900", letterSpacing: 0.7, paddingTop: 12, paddingBottom: 4 }, recapRow: { flexDirection: "row", alignItems: "center", gap: 11, paddingVertical: 11 }, recapRowBorder: { borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: "#F0F3F8" }, recapBody: { flex: 1, minWidth: 0 }, recapLabel: { color: "#667085", fontSize: 9.5, fontWeight: "900", letterSpacing: 0.6 }, recapValue: { color: "#1F1206", fontSize: 14, fontWeight: "800", marginTop: 2 }, recapEdit: { color: "#9A6201", fontSize: 12, fontWeight: "800" }, brandStatus: { width: 6, height: 6, borderRadius: 3, backgroundColor: "#18A572" }, brandChipText: { color: "#9A6201", fontSize: 10, fontWeight: "900", letterSpacing: 0.45 }, trustList: { marginTop: 23, gap: 10 }, trustRow: { flexDirection: "row", gap: 10, padding: 12, backgroundColor: "#FFFFFF", borderRadius: 16, borderWidth: 1, borderColor: "#F0F3F8" }, trustIcon: { width: 36, height: 36, borderRadius: 12, backgroundColor: "#F0F3F8", alignItems: "center", justifyContent: "center" }, trustInfo: { flex: 1 }, trustTitle: { color: "#1F1206", fontSize: 13, fontWeight: "900" }, trustText: { color: "#667085", fontSize: 12, lineHeight: 17, marginTop: 2 }, legal: { color: "#667085", fontSize: 11, lineHeight: 17, textAlign: "center", marginTop: 14, paddingHorizontal: 12 }, legalLink: { color: "#9A6201", fontWeight: "900" }, top: { height: 49, flexDirection: "row", alignItems: "center", marginBottom: 25 }, backButton: { width: 42, height: 42, borderRadius: 15, backgroundColor: "#FFFFFF", borderColor: "#F0F3F8", borderWidth: 1, alignItems: "center", justifyContent: "center" }, stepInfo: { flex: 1, marginLeft: 10 }, stepLabel: { color: "#9A6201", fontSize: 10, fontWeight: "900", letterSpacing: 0.8 }, stepCount: { color: "#1F1206", fontSize: 13, fontWeight: "900", marginTop: 2 }, stepDots: { flexDirection: "row", gap: 4 }, stepDot: { width: 7, height: 7, borderRadius: 4, backgroundColor: "#D6DCE6" }, stepDotActive: { backgroundColor: "#9A6201" }, form: { flex: 1, paddingTop: 5 }, heroIcon: { width: 64, height: 64, borderRadius: 22, alignItems: "center", justifyContent: "center", backgroundColor: "#F0F3F8", marginBottom: 20 }, title: { color: "#1F1206", fontSize: 28, lineHeight: 34, fontWeight: "900", letterSpacing: -0.5 }, subtitle: { color: "#667085", fontSize: 14, lineHeight: 21, marginTop: 8 }, fieldLabel: { color: "#667085", fontSize: 11, fontWeight: "900", letterSpacing: 0.75, marginTop: 25, marginBottom: 8 }, countryField: { minHeight: 60, borderRadius: 16, paddingHorizontal: 12, flexDirection: "row", alignItems: "center", backgroundColor: "#FFFFFF", borderColor: "#E3E3E3", borderWidth: 1 }, countryBadge: { width: 36, height: 36, borderRadius: 12, alignItems: "center", justifyContent: "center", backgroundColor: "#FFFFFF" }, countryFlag: { color: "#9A6201", fontWeight: "900", fontSize: 11 }, countryInfo: { flex: 1, marginLeft: 10 }, countryName: { color: "#9A6201", fontSize: 14, fontWeight: "900" }, countryHint: { color: "#667085", fontSize: 11, marginTop: 2 }, phoneField: { height: 58, borderRadius: 16, backgroundColor: "#FFFFFF", borderWidth: 1, borderColor: "#E3E3E3", flexDirection: "row", alignItems: "center", paddingHorizontal: 14 }, fieldError: { borderColor: "#E45858" }, dialCode: { paddingRight: 12, borderRightWidth: 1, borderColor: "#F0F3F8" }, dialCodeText: { color: "#9A6201", fontWeight: "900", fontSize: 15 }, phoneInput: { flex: 1, color: "#9A6201", fontSize: 17, fontWeight: "800", marginLeft: 12, letterSpacing: 0.5 }, helper: { color: "#667085", fontSize: 12, lineHeight: 18, marginTop: 8 }, error: { color: "#A43740", fontSize: 12, lineHeight: 18, marginTop: 8, fontWeight: "700" }, actionButton: { marginTop: 25 }, simulationNotice: { flexDirection: "row", gap: 9, padding: 12, borderRadius: 15, backgroundColor: "#F0F3F8", marginTop: 22 }, simulationWarning: { backgroundColor: "#FBEAEC", borderWidth: 1, borderColor: "#E7B9BE" }, simulationText: { flex: 1, color: "#9A6201", fontSize: 12, lineHeight: 18 }, simulationWarningText: { color: "#8C2F37" }, simulationCode: { color: "#9A6201", fontWeight: "900" }, phoneHighlight: { color: "#9A6201", fontWeight: "900" }, otpRow: { flexDirection: "row", justifyContent: "space-between", gap: 8, marginTop: 27 }, otpInput: { flex: 1, minWidth: 0, height: 54, backgroundColor: "#FFFFFF", borderRadius: 14, borderWidth: 1, borderColor: "#E3E3E3", color: "#9A6201", fontSize: 21, fontWeight: "900" }, otpError: { borderColor: "#E45858" }, resend: { alignItems: "center", paddingVertical: 17 }, resendText: { color: "#9A6201", fontSize: 13, fontWeight: "900" }, resendDisabled: { color: "#667085" }, roleChoice: { marginTop: 17, padding: 15, minHeight: 103, borderRadius: 19, backgroundColor: "#FFFFFF", borderWidth: 1, borderColor: "#F0F3F8", flexDirection: "row", alignItems: "center", gap: 12 }, roleChoiceActive: { borderColor: "#9A6201", backgroundColor: "#F0F3F8" }, roleChoiceIcon: { width: 51, height: 51, borderRadius: 17, backgroundColor: "#F0F3F8", alignItems: "center", justifyContent: "center" }, roleChoiceIconActive: { backgroundColor: "#9A6201" }, roleChoiceInfo: { flex: 1 }, roleChoiceTitle: { color: "#1F1206", fontSize: 16, fontWeight: "900" }, roleChoiceText: { color: "#667085", fontSize: 12, lineHeight: 17, marginTop: 4 }, radio: { width: 22, height: 22, borderRadius: 11, borderWidth: 2, borderColor: "#F0F3F8", alignItems: "center", justifyContent: "center" }, radioActive: { borderColor: "#9A6201" }, radioInner: { width: 11, height: 11, borderRadius: 6, backgroundColor: "#9A6201" }, vehicleGrid: { flexDirection: "row", flexWrap: "wrap", gap: 10, marginTop: 22 }, vehicleCard: { width: "48.5%", minHeight: 130, borderRadius: 18, padding: 13, backgroundColor: "#FFFFFF", borderWidth: 1, borderColor: "#F0F3F8" }, vehicleCardActive: { backgroundColor: "#9A6201", borderColor: "#9A6201" }, vehicleTitle: { color: "#1F1206", fontSize: 15, fontWeight: "900", marginTop: 11 }, vehicleTitleActive: { color: "#FFFFFF" }, vehicleText: { color: "#667085", fontSize: 11, marginTop: 3 }, vehicleTextActive: { color: "#F0F3F8" }, vehicleCheck: { width: 20, height: 20, borderRadius: 10, borderWidth: 1.5, borderColor: "#F0F3F8", alignItems: "center", justifyContent: "center", position: "absolute", right: 10, top: 10 }, vehicleCheckActive: { backgroundColor: "#18A572", borderColor: "#18A572" }, nameInput: { minHeight: 58, paddingHorizontal: 15, borderRadius: 16, borderWidth: 1, borderColor: "#E3E3E3", backgroundColor: "#FFFFFF", color: "#9A6201", fontSize: 16, fontWeight: "800" }, referralToggle: { flexDirection: "row", alignItems: "center", gap: 7, marginTop: 20, alignSelf: "flex-start" }, referralToggleText: { color: "#9A6201", fontSize: 13, fontWeight: "800" }, lockedRole: { flexDirection: "row", alignItems: "flex-start", gap: 8, padding: 12, borderRadius: 14, backgroundColor: "#F0F3F8", marginTop: 18 }, lockedRoleText: { flex: 1, color: "#9A6201", fontSize: 12, lineHeight: 18 }, modal: { flex: 1, justifyContent: "flex-end" }, modalScrim: { ...StyleSheet.absoluteFill, backgroundColor: "rgba(11,31,58,0.46)" }, countrySheet: { backgroundColor: "#FFFFFF", borderTopLeftRadius: 26, borderTopRightRadius: 26, padding: 20, paddingBottom: 30 }, sheetHandle: { width: 40, height: 4, borderRadius: 2, backgroundColor: "#F0F3F8", alignSelf: "center", marginBottom: 18 }, sheetTop: { flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }, sheetTitle: { color: "#1F1206", fontSize: 20, fontWeight: "900" }, sheetSubtitle: { color: "#667085", fontSize: 12, marginTop: 3 }, sheetClose: { width: 38, height: 38, borderRadius: 13, alignItems: "center", justifyContent: "center", backgroundColor: "#FFFFFF" }, countryRow: { minHeight: 58, flexDirection: "row", alignItems: "center", paddingHorizontal: 8, borderRadius: 15, marginTop: 4 }, countryRowActive: { backgroundColor: "#FFFFFF" }, pressed: { opacity: 0.68 },
});
