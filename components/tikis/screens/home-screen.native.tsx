...
import { ActivityIndicator, Animated, Dimensions, PanResponder, Pressable, ScrollView, StyleSheet, Text, View } from "react-native";
...
const SHEET_EXPANDED = Math.min(SCREEN_H * 0.72, 620);
...
export function HomeScreen() {
  ...
  useEffect(() => {
    if (selectedId && focusedList.every((d) => d.id !== selectedId) && urgent) setSelectedId(urgent.id);
  ...
  const selected = deliveries.find((d) => d.id === selectedId) ?? urgent;
  ...
  return (
    <SafeAreaView style={styles.safe} edges={["top", "bottom"]}>
      <MapView
        ...
        initialRegion={{
          latitude: focusedList[0]?.pickup.latitude ?? 12.3714,
          longitude: focusedList[0]?.pickup.longitude ?? -1.5197,
          latitudeDelta: 0.15,
          longitudeDelta: 0.15,
        }}
      >
        {focusedList.slice(0, 5).map((d) => [
          <Marker key={`p-${d.id}`} coordinate={{ latitude: d.pickup.latitude, longitude: d.pickup.longitude }} anchor={{ x: 0.5, y: 0.5 }}>
            <View style={styles.nativeMarkerStart}><MaterialIcons name="inventory-2" size={14} color="#FFFFFF" /></View>
          </Marker>,
          <Marker key={`d-${d.id}`} coordinate={{ latitude: d.dropoff.latitude, longitude: d.dropoff.longitude }} anchor={{ x: 0.5, y: 0.85 }}>
            <View style={styles.nativeMarkerEnd}><MaterialIcons name="location-on" size={16} color="#B4232D" /></View>
          </Marker>,
        ])}
        {selected ? (
          <Marker coordinate={{ latitude: selected.pickup.latitude, longitude: selected.pickup.longitude }} anchor={{ x: 0.5, y: 0.5 }}>
            <View style={styles.nativeMarkerDriver}><MaterialIcons name="two-wheeler" size={16} color="#FFFFFF" /></View>
          </Marker>
        ) : null}
      </MapView>
...
const styles = StyleSheet.create({
  ...
  fab: { position: "absolute", right: 14, bottom: 320, width: 50, height: 50, borderRadius: 14, backgroundColor: "#007B8B", alignItems: "center", justifyContent: "center", shadowColor: "#007B8B", shadowOpacity: 0.4, shadowRadius: 8, shadowOffset: { width: 0, height: 4 }, elevation: 6, zIndex: 10 },
  ...
  nativeMarkerStart: { width: 28, height: 28, borderRadius: 14, backgroundColor: "#007B8B", alignItems: "center", justifyContent: "center", borderWidth: 2, borderColor: "#FFFFFF" },
  nativeMarkerEnd: { width: 28, height: 28, borderRadius: 14, backgroundColor: "#FFFFFF", alignItems: "center", justifyContent: "center", borderWidth: 2, borderColor: "#B4232D" },
  nativeMarkerDriver: { width: 32, height: 32, borderRadius: 16, backgroundColor: "#111111", alignItems: "center", justifyContent: "center", borderWidth: 3, borderColor: "#FFFFFF" },
});
