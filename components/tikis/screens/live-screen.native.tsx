...
const SHEET_EXPANDED = Math.min(SCREEN_H * 0.72, 600);
...
export function LiveScreen() {
  ...
  useEffect(() => {
    if (visible.every((d) => d.id !== selectedId)) setSelectedId(visible[0]?.id ?? null);
  }, [visible, selectedId]);
  ...
  return (
    <SafeAreaView style={styles.safe} edges={["top", "bottom"]}>
      <MapView
        ...
        initialRegion={{
          latitude: visible[0]?.pickup.latitude ?? 12.3714,
          longitude: visible[0]?.pickup.longitude ?? -1.5197,
          latitudeDelta: 0.12,
          longitudeDelta: 0.12,
        }}
      >
      ...
      <View style={styles.fab} pointerEvents="box-none">
        <Pressable onPress={() => selected && recenter({ latitude: selected.pickup.latitude, longitude: selected.pickup.longitude, latitudeDelta: 0.06, longitudeDelta: 0.06 })} style={({ pressed }) => [styles.iconBtn, pressed && styles.pressed]} accessibilityLabel="Recentrer">
...
const styles = StyleSheet.create({
  ...
  fab: { position: "absolute", right: 14, bottom: 160 },
