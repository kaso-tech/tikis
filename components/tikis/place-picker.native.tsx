...
export function PlacePicker({ label, tone, value, countryCode, onChange }: Props) {
  ...
  const accent = tone === "pickup" ? "#9A6201" : "#B4232D";
  ...
  return <View style={styles.wrap}>
    ...
    <SurfaceCard style={styles.card}>
      <View style={styles.searchRow}>
        ...
        <TextInput value={query} onChangeText={(text) => { setQuery(sanitizePlaceText(text, 120, { preserveTrailingSpace: true })); setMessage(""); }} maxLength={120} placeholder="Adresse, commerce ou lieu public" placeholderTextColor="#B48753" returnKeyType="search" onSubmitEditing={() => void runSearch(query, true)} style={styles.input} />
        <Pressable accessibilityRole="button" accessibilityLabel="Prioriser les résultats autour de ma position" onPress={() => void prioritizeNearbyResults()} disabled={gpsStatus === "loading"} style={({ pressed }) => [styles.nearbyButton, (pressed || gpsStatus === "loading") && styles.pressed]}>{gpsStatus === "loading" ? <ActivityIndicator size="small" color="#9A6201" /> : <MaterialIcons name={deviceBias ? "my-location" : "near-me"} size={18} color="#9A6201" />}</Pressable>
        <Pressable accessibilityRole="button" accessibilityLabel={`Rechercher ${label}`} onPress={() => void runSearch(query, true)} disabled={search.isPending} style={({ pressed }) => [styles.searchButton, (pressed || search.isPending) && styles.pressed]}>{search.isPending ? <ActivityIndicator size="small" color="#9A6201" /> : <MaterialIcons name="search" size={19} color="#9A6201" />}</Pressable>
      </View>
      <Pressable accessibilityRole="button" accessibilityLabel="Utiliser ma position actuelle" onPress={() => void selectCurrentLocation()} disabled={gpsStatus === "loading" || reverse.isPending} style={({ pressed }) => [styles.currentLocationButton, (pressed || gpsStatus === "loading" || reverse.isPending) && styles.pressed]}>{gpsStatus === "loading" || reverse.isPending ? <ActivityIndicator size="small" color="#9A6201" /> : <MaterialIcons name="my-location" size={18} color="#9A6201" />}<Text style={styles.currentLocationText}>{reverse.isPending ? "Identification de votre position…" : "Utiliser ma position actuelle"}</Text></Pressable>
