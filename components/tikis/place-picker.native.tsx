...
export function PlacePicker({ label, tone, value, onChange }: Props) {
  ...
  async function runSearch() {
    const clean = sanitizePlaceText(query);
    setQuery(clean);
    if (clean.length < 2) { setMessage("Saisissez au moins deux caractères."); return; }
    try {
      setMessage("");
    ...
    } catch (cause) { setMessage(cause instanceof Error ? cause.message : "La recherche est indisponible."); }
  }
