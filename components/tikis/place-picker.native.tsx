  ...
  return <View style={styles.wrap}>
    ...
    <SurfaceCard style={styles.card}>
      <View style={styles.searchRow}>
        ...
        <TextInput value={query} onChangeText={(text) => { setQuery(sanitizePlaceText(text, 120, { preserveTrailingSpace: true })); setMessage(""); }} maxLength={120} placeholder="Adresse, commerce ou lieu public" placeholderTextColor={theme.muted} returnKeyType="search" onSubmitEditing={() => void runSearch(query, true)} style={styles.input} />
        <Pressable accessibilityRole="button" accessibilityLabel="Prioriser les résultats autour de ma position" onPress={() => void prioritizeNearbyResults()} disabled={gpsStatus === "loading"} style={({ pressed }) => [styles.nearbyButton, (pressed || gpsStatus === "loading") && styles.pressed]}>{gpsStatus === "loading" ? <ActivityIndicator size="small" color={theme.primary} /> : <MaterialIcons name={deviceBias ? "my-location" : "near-me"} size={18} color={theme.primary} />}</Pressable>
        <Pressable accessibilityRole="button" accessibilityLabel={`Rechercher ${label}`} onPress={() => void runSearch(query, true)} disabled={search.isPending} style={({ pressed }) => [styles.searchButton, (pressed || search.isPending) && styles.pressed]}>{search.isPending ? <ActivityIndicator size="small" color={theme.primary} /> : <MaterialIcons name="search" size={19} color={theme.primary} />}</Pressable>
      </View>
      <Pressable accessibilityRole="button" accessibilityLabel="Utiliser ma position actuelle" onPress={() => void selectCurrentLocation()} disabled={gpsStatus === "loading" || reverse.isPending} style={({ pressed }) => [styles.currentLocationButton, (pressed || gpsStatus === "loading" || reverse.isPending) && styles.pressed]}>{gpsStatus === "loading" || reverse.isPending ? <ActivityIndicator size="small" color={theme.primary} /> : <MaterialIcons name="my-location" size={18} color={theme.primary} />}<Text style={styles.currentLocationText}>{reverse.isPending ? "Identification de votre position…" : "Utiliser ma position actuelle"}</Text></Pressable>
      <View style={styles.mapWrap}><MapView provider={PROVIDER_GOOGLE} ref={mapRef} style={styles.map} initialRegion={value ? { latitude: value.latitude, longitude: value.longitude, latitudeDelta: 0.025, longitudeDelta: 0.025 } : INITIAL_REGION} onPress={(event) => void selectOnMap(event)}>{value ? <Marker coordinate={{ latitude: value.latitude, longitude: value.longitude }} pinColor={accent} title={locationTitle(value)} description={locationSubtitle(value)} /> : null}</MapView><View pointerEvents="none" style={styles.mapHint}><MaterialIcons name="touch-app" size={14} color={theme.surface} /><Text style={styles.mapHintText}>Touchez la carte pour sélectionner</Text></View></View>
