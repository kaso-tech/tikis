...
- [x] Relancer Expo et l’API, puis vérifier les services.
- [x] Enregistrer, pousser et livrer la dernière version validée.
- [x] Aligner les dates des pages admin Livraisons, Finance et Parrainage sur les objets `Date` retournés par tRPC.
- [x] Ajouter les métadonnées `bonus` et `penalty` au mapping Wallet.
- [x] Revalider TypeScript, les tests, le build admin et les services après correction.
- [x] Enregistrer, pousser et livrer la version synchronisée corrigée.
...
- [x] Relancer Expo et l’API, puis vérifier les services.
- [x] Enregistrer, pousser et livrer la dernière version validée.
- [x] Aligner les coordonnées du suivi en direct sur le contrat attendu par la carte.
- [x] Corriger la comparaison de rôle impossible dans le routeur de livraisons.
- [x] Revalider TypeScript, les tests, le build admin et les services après ces corrections.
- [x] Rétablir le bundling Metro après l’erreur de génération `.expo/types/router.d.ts`.
- [x] Revalider Metro, l’API, `/admin`, TypeScript et les tests après ce correctif.
- [x] Isoler le suivi cartographique natif de la route web afin d’éviter le chargement de `react-native-maps` sur web.
